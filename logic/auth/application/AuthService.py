...
class JwtAuthService(AuthUseCase):
    ...
    def login(self, username, password):
        ...
        if user.compare_password(password) is False:
            raise exceptions.SigninFail

        access_token = self._create_access_token(user.id, user.nickname, secret_key)
        refresh_token = self._create_refresh_token(secret_key)
        self.token_dao.save(user.id, access_token, refresh_token)
    ...
    def refresh(self, refresh_token) -> str:
        ...
        user = token['user']
        access_token = self._create_access_token(user['_id'], user['nickname'], secret_key)
